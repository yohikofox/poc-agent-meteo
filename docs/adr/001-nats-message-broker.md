...
La plateforme repose sur 5 agents autonomes qui doivent communiquer entre eux de façon découplée. L'orchestrator doit envoyer une tâche à un agent et attendre sa réponse avant de passer à l'étape suivante.
...
- Le `WeatherReportOrchestrator` appelle `nc.request(subject, payload, { headers, timeout })` et bloque jusqu'à la réponse.
